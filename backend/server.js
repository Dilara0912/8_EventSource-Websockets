...
const userState = []; // здесь хранятся все пользователи
app.post("/new-user", async (request, response) => {
  ...
  // если нет — создает нового и возвращает { status: "ok", user: {...} }
    // если есть — возвращает { status: "error", message: "This name is already taken!" }
  const isExist = userState.find((user) => user.name === name);
  if (!isExist) {
    const newUser = {
      id: randomUUID(), // сервер генерирует уникальный ID
      name: name,
    };
    userState.push(newUser); // добавляем пользователя в список
    const result = {
      status: "ok",
      user: newUser,
    };
...
const port = process.env.PORT || 3000; //Сервер работает на порту 3000
